...
prerelease: vendor
	...
	$(MAKE) prerelease-upload GOOS=linux GOARCH=amd64 VERSION_ID=$$\(BRANCH_ID\)
	$(eval tag := $(shell git describe --tags --exact-match HEAD))
	$(if $(tag),$(MAKE) release)
	$(if $(tag),$(MAKE) prerelease-upload GOOS=darwin GOARCH=amd64)
	$(if $(tag),$(MAKE) prerelease-upload GOOS=darwin GOARCH=amd64 ENTERPRISE=1)
	$(if $(tag),$(MAKE) prerelease-upload GOOS=linux GOARCH=amd64)
	$(if $(tag),$(MAKE) prerelease-upload GOOS=linux GOARCH=amd64 ENTERPRISE=1)
	$(if $(tag),$(MAKE) prerelease-upload GOOS=linux GOARCH=386)
	$(if $(tag),$(MAKE) prerelease-upload GOOS=linux GOARCH=386 ENTERPRISE=1)

prerelease-upload: build/pilosa-$(VERSION_ID).tar.gz
	aws s3 cp build/pilosa-$(VERSION_ID).tar.gz s3://build.pilosa.com/pilosa-$(VERSION_ID).tar.gz --acl public-read
